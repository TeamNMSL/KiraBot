...
namespace KiraDX.Bot.arcaea
{
    ...
    {
        static void ArcPic(DisMsg g, string username, string songname, string score, string diff, string friendcode, string userptt, string pure, string shinepure, string far, string lost, string songptt, string playtime, string type)
        {
            ...
            KiraPlugin.SendPicAsync($"{G.path.Apppath}{G.path.ArcScore}{username}.jpg", g);
        }
        ...
        static void ArcB30(b30info info, GroupMsg g)
        {
            try
            {
                ...
                using (Graphics graphics = Graphics.FromImage(Pic))
                {
                    ...
                    graphics.DrawString(File.ReadAllText($"{G.path.Apppath}{G.path.ArcUser}{g.fromAccount}.ini"), UnFCF, Brushes.White, ufc);
        ...
        static void ArcB30(b30info info, DisMsg g)
        {
            try
            {
                string GetFilename_Songcover(string songname, string diff)
                ...
                myEncoderParameters.Param[0] = myEncoderParameter;
                Pic.Save($"{G.path.Apppath}{G.path.ArcScore}{username}_b30.jpg", myImageCodecInfo, myEncoderParameters);
                KiraPlugin.SendPicAsync($"{G.path.Apppath}{G.path.ArcScore}{username}_b30.jpg", g);
